...
max_bit_count = {8192: 438, 8192*2: 881}
...
def re_depth(encoder, evaluator, relin_keys, ctxt_list, count):
    ...
    for ctxt in ctxt_list:
        for _ in range(count):
            encoded_coeff = encoder.encode(coeff, ctxt.scale())
...
def average_pooling_layer_converter(evaluator, encoder, galois_key, relin_keys, ctxt_list,
                                    kernel_size, input_size, real_input_size, padding, stride, tmp_param, data_size, const_param):
    result_list = []
    rotated_ctxt_list = []
...
def fc_layer_converter(evaluator, encoder, galois_key, relin_keys, ctxt, weights, bias, data_size:int=400):
    ...
    for idx in range(output_size):
        ...
        rotate_in_subspace(evaluator, encoder, galois_key, relin_keys, result_list, weight, ctxt, idx, rot_interval,
                           num_of_slots=num_of_slot, data_size=data_size)
...
def approximated_ReLU_converter(evaluator, encoder, input_size, real_size, relin_keys, ctxt_list, _type=0, const_param=1):
    ...
    # eff1 * ctxt^2 + eff2 * ctxt + eff3
    # Highly accurate cnn inference using approximate activation functions over homomorphic
    # encryption. In Proc. of the International Conference on Big Data (Big
    # Data), pages 3989–3995, 2020.
    if _type == 0:
        if type(ctxt_list) == list:
            ...
            ctxt = ctxt_list
            encoded_coeff1 = encoder.encode(coeff1, ctxt.scale())# scale)
            evaluator.mod_switch_to_inplace(encoded_coeff1, ctxt.parms_id())
            temp = evaluator.multiply_plain(ctxt, encoded_coeff1) # eff1 * ctxt, s^2, a
            evaluator.relinearize_inplace(temp, relin_keys)
            # evaluator.rescale_to_next_inplace(temp) # s, a-1
            # ctxt = re_depth(encoder, evaluator, relin_keys, [ctxt], 1)[0] # s, a-1

            # print('chain index', context.get_context_data(temp.parms_id()).chain_index(),
            #       'chain index', context.get_context_data(ctxt.parms_id()).chain_index())

            encoded_coeff2 = encoder.encode(coeff2, temp.scale())
            evaluator.mod_switch_to_inplace(encoded_coeff2, temp.parms_id())
            temp = evaluator.add_plain(temp, encoded_coeff2) # eff1 * ctxt + eff2

            temp = evaluator.multiply(ctxt, temp) # eff1 * ctxt^2 + eff2 * ctxt
            evaluator.relinearize_inplace(temp, relin_keys)
            # evaluator.rescale_to_next_inplace(temp)

            encoded_coeff3 = encoder.encode(coeff3, temp.scale())
            evaluator.mod_switch_to_inplace(encoded_coeff3, temp.parms_id())
            temp = evaluator.add_plain(temp, encoded_coeff3) # eff1 * ctxt^2 + eff2 * ctxt + eff3
...
def rotate_in_subspace(evaluator, encoder, galois_key, relin_keys, result_list:list, weight, ctxt, rot_n:int,
                       interval_len:int, num_of_slots:int=num_of_slot, data_size:int=400):
