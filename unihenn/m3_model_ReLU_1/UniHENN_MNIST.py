...
modulus_chain_length = min(max_medium_scale, 9)
...
print(f'max_medium_scale: {max_medium_scale}')
...
image_size = 28
data_size = 1080
num_of_data = 6 #int((poly_modulus_degree/2)//data_size)
...
def enc_test(evaluator, ckks_encoder, galois_key, relin_keys, csps_ctxt, csps_conv_weights, csps_conv_biases,
    ...
    # result, const_param = approximated_ReLU_converter(evaluator, ckks_encoder, data_size, data_size, relin_keys, result,
    #                                                   0, const_param)
    # CHECK_TIME2 = time.time()
    # print('APPROX ReLU TIME', CHECK_TIME2 - CHECK_TIME1, 'scale', result[0].scale(),
    #       'chain index', context.get_context_data(result[0].parms_id()).chain_index(),
    #       'tmp_param', S, 'const_param', const_param, 'OH', OH)
    ...
    result: seal.Ciphertext = fc_layer_converter(evaluator, ckks_encoder, galois_key, relin_keys, result,
    ...
    END_TIME = time.time()
    print('FC2 TIME', END_TIME - CHECK_TIME6)
