...
modulus_chain_length = min(max_medium_scale, 13)
...
print('chain_length', modulus_chain_length)
...
csps_fc_weights.append(model_cnn['FC4.weight'])
csps_fc_biases.append(model_cnn['FC4.bias'])
strides = [1, 1, 1]
paddings = [0, 0, 0]
...
for i in range(len(csps_fc_weights)):
    print(f"FC{i+1}.weight:\t", csps_fc_weights[i].shape)
    print(f"FC{i+1}.bias:\t", csps_fc_biases[i].shape)
print()


image_size = 28 # Suppose that image shape is sqaure
# data_size must be larger than first Dense layer input size to rotate ciphertext
# data_size = image_size**2 + image_size + 1 # 1800
data_size = 1080
# num_of_data adjust to fit poly_modulus_degree
...
def enc_test(evaluator, ckks_encoder, galois_key, relin_keys, csps_ctxt, csps_conv_weights, csps_conv_biases,
    ...
    result, const_param = approximated_ReLU_converter(evaluator, ckks_encoder, data_size, 1024, relin_keys, result, 0,
                                                      1)
    ...
    print('FC3 TIME', CHECK_TIME9 - CHECK_TIME8)

    result, const_param = approximated_ReLU_converter(evaluator, ckks_encoder, data_size, 1024, relin_keys, result, 0,
                                                      1)
    CHECK_TIME10 = time.time()
    print('APPROX ReLU TIME', CHECK_TIME10 - CHECK_TIME9)

    result = fc_layer_converter(evaluator, ckks_encoder, galois_key, relin_keys, result,
                                csps_fc_weights[3], csps_fc_biases[3], data_size=data_size)
    END_TIME = time.time()
    print('FC4 TIME', END_TIME - CHECK_TIME8)
    ...
    print(
        'Test Accuracy (Overall): {0}% ({1}/{2})'.format(count_correct / num_of_data * 100, count_correct, num_of_data))
