...
HeBNN::HeBNN(FILE* in_file, bool b_prep, const char *key)
{
	HeBNN::init(in_file, false, key) ;
}

HeBNN::HeBNN(const char *model, const char *key)
{
	//open up input file
	FILE *fd = fopen(model, "r");
	if(fd == NULL)
	{
		printf("Bad Weights File. Exiting...\r\n") ;
		return ;
	}
	HeBNN::init(fd, false, key) ;
	fclose(fd) ;
}

void HeBNN::init(FILE* in_file, bool b_prep, const char *key)
{
	tDimensions* p_dim ;
	tDimensions lay_dim ;
	uint32_t conv_dep ;
